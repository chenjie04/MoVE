...
class CrissCrossAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        att_H = concate[:,:,:,0:height].permute(0,2,1,3).contiguous().view(m_batchsize*width,height,height).type_as(proj_value_H)
        att_W = concate[:,:,:,height:height+width].contiguous().view(m_batchsize*height,width,width).type_as(proj_value_W)
...
if __name__ == '__main__':
    ...
    model = CrissCrossAttention(64).to(device)
    x = torch.randn(2, 64, 5, 6).to(device)
    out = model(x)
    print(out.shape)